...
GO
CREATE TABLE FrameworkLanguage
(
    Id INT PRIMARY KEY IDENTITY,
    AppTypeName NVARCHAR(32) NOT NULL, -- Key
    Name NVARCHAR(512) NOT NULL, -- Key
    TextHtml NVARCHAR(256),
    INDEX IX_FrameworkLanguage UNIQUE (AppTypeName, Name)
)
GO
CREATE VIEW FrameworkLanguageIntegrate AS
SELECT
    *,
    Name AS IdName
FROM
...
GO
CREATE TABLE FrameworkLanguageItem
(
    Id INT PRIMARY KEY IDENTITY,
    AppTypeName NVARCHAR(32) NOT NULL, -- Key
    Name NVARCHAR(512) NOT NULL, -- Key
    TextDefault NVARCHAR(512),
    INDEX IX_FrameworkLanguageItem UNIQUE (AppTypeName, Name)
)
GO
CREATE VIEW FrameworkLanguageItemIntegrate AS
SELECT
    *,
    CONCAT(AppTypeName, '; ', Name) AS IdName
FROM
    FrameworkLanguageItem

GO
CREATE TABLE FrameworkLanguageText
(
    Id INT PRIMARY KEY IDENTITY,
    AppTypeName NVARCHAR(32) NOT NULL, -- Key
    LanguageId INT FOREIGN KEY REFERENCES FrameworkLanguage(Id) NOT NULL,
    ItemId INT FOREIGN KEY REFERENCES FrameworkLanguageItem(Id) NOT NULL ,
    Text NVARCHAR(512), -- Text default
    INDEX IX_FrameworkLanguageText UNIQUE (AppTypeName, LanguageId, ItemId)
)
GO
CREATE VIEW FrameworkLanguageTextIntegrate AS
SELECT
    Text.*,
    (SELECT Name FROM FrameworkLanguage Language WHERE Language.Id = Text.LanguageId) AS LanguageIdName,
    (SELECT CONCAT(Item.AppTypeName, '; ', Item.Name) FROM FrameworkLanguageItem Item WHERE Item.Id = Text.ItemId) AS ItemIdName
FROM
    FrameworkLanguageText Text

GO
CREATE VIEW FrameworkLanguageApp AS
SELECT
    Language.Id AS LanguageId,
    Language.AppTypeName AS LanguageAppTypeName,
    Language.Name AS LanguageName,
    Item.Id AS ItemId,
    Item.Name AS ItemName,
    Item.TextDefault AS ItemTextDefault,
    Text.Id AS TextId,
    Text.Text AS TextText
FROM
    FrameworkLanguage Language,
    FrameworkLanguageItem Item,
    FrameworkLanguageText Text
WHERE
    Text.LanguageId = Language.Id AND
	Text.ItemId = Item.Id

GO
CREATE VIEW FrameworkLanguageDisplay AS
SELECT
    Language.Id AS LanguageId,
    Language.AppTypeName AS LanguageAppTypeName,
    Language.Name AS LanguageName,
...
FROM
    FrameworkLanguage Language
INNER JOIN
    FrameworkLanguageItem Item ON (Language.AppTypeName = Item.AppTypeName)
LEFT JOIN
    FrameworkLanguageText Text ON (Text.LanguageId = Language.Id AND Text.ItemId = Item.Id)
